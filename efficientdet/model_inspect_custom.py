...
class Inspector(object):
  def __init__(self,
      ...
      logdir='/tmp/deff/', #log directory
      runmode='dry', # 'Run mode: {freeze, bm, dry}'
      trace_filename= None, # 'Trace file name.'

      threads= 0,# 'Number of threads.')
      bm_runs= 10, # 'Number of benchmark runs.')
      tensorrt= None, # 'TensorRT mode: {None, FP32, FP16, INT8}')
      delete_logdir= True, # 'Whether to delete logdir.')
      freeze= False, #'Freeze graph.')
      use_xla= False, #'Run with xla optimization.')
      batch_size= 1, #'Batch size for inference.')

      ckpt_path= None, # 'checkpoint dir used for eval.')
      export_ckpt= None, # 'Path for exporting new models.')

      hparams= '', # 'Comma separated k=v pairs of hyperparameters or a module containing attributes to use as hyperparameters.')

      input_image= None, #'Input image path for inference.')
      output_image_dir= None, # 'Output dir for inference.')

      # For video.
      input_video= None, # 'Input video path for inference.')
      output_video= None,# 'Output video path. If None, play it online instead.')

      # For visualization.
      line_thickness= None, # 'Line thickness for box.')
      max_boxes_to_draw= 100, # 'Max number of boxes to draw.')
      min_score_thresh= 0.4, # 'Score threshold to show box.')
      nms_method= 'hard', # 'nms method, hard or gaussian.')

      # For saved model.
      saved_model_dir= '/tmp/saved_model',# 'Folder path for saved model.')
      tflite_path= None): #'Path for exporting tflite file.')
    ...
    self.bm_runs= bm_runs
    self.tensorrt=tensorrt
    self.delete_logdir=delete_logdir
    self.freeze=freeze
  ...
  def get_model_inspector(self):
    ...
    inspector = ModelInspector(
        ...
        score_thresh=self.min_score_thresh,
        max_output_size=self.max_boxes_to_draw,
